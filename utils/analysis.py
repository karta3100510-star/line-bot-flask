...
def _cache_set(key,val,ttl=_TTL): _CACHE[key]=(_now(),ttl,val)
def _import_yf():
    global _yf
    if _yf is None:
...
def fetch_quote(ticker:str)->Dict[str,Any]:
    key=f"q:{ticker}"; c=_cache_get(key)
    if c is not None: return c
    yf=_import_yf(); info={'ticker':ticker}
    try:
        t=yf.Ticker(ticker)
        d=t.history(period='5d', interval='1d', auto_adjust=False)
        if not d.empty:
            last=float(d['Close'].iloc[-1]); prev=float(d['Close'].iloc[-2]) if len(d)>=2 else None
            info['price']=last; info['chg_1d_pct']=_pct(prev,last) if prev is not None else None
...
def analyze_text(text:str)->Dict[str,Any]:
    tickers=extract_tickers(text); quotes=[]
    for t in tickers:
