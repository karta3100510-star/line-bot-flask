...
SOCIAL_CONFIG = [
    {'type': 'facebook', 'page_id': '16gt5rCZJD'},      # 支援 share id 或數字 id
...
def _graph_lookup_og_object_id(url: str) -> str:
    try:
        resp = requests.get('https://graph.facebook.com/v17.0/',
...
def _graph_post_to_page_id(post_id: str) -> str:
    try:
        resp = requests.get(f'https://graph.facebook.com/v17.0/{post_id}',
...
def fetch_facebook_posts(page_id_or_share: str, limit: int = 2):
    ...
    try:
        ...
        data = [{'text': _clean_text(p.get('message', '')),
        ...
        try:
            if cfg['type'] == 'facebook':
                ...
                if isinstance(note, str) and note:
                    results.append({'source': f"Facebook {cfg['page_id']}",
                                    'text': f"[info: {note}]",
                                    'url': '',
                ...
                for p in posts:
                    ...
                    results.append({'source': f"Facebook {cfg['page_id']}",
                                    ...
                                    'time': p['time'], 'analysis': analysis})
            elif cfg['type'] == 'substack':
                posts, err = fetch_substack_posts(cfg['handle'])
                if err:
                    results.append({'source': f"Substack {cfg['handle']}",
                                    'text': f"[{err}]",
                                    'url': '',
                                    'time': datetime.utcnow().isoformat(),
                                    'analysis': {}})
            ...
            results.append({'source': cfg.get('page_id') or cfg.get('handle'),
                            'text': f"[抓取失敗: {e}]",
                            'url': '', 'time': datetime.utcnow().isoformat(),
                            'analysis': {}})
    # 永遠寫檔
    try:
